...
# vreplay/tests/expected/<name>.dump.  A final smoke test streams one case
...
# at the in-tree stdlib and vreplay library (see README.md here)
...
OCAMLC="runtime/ocamlrun ./ocamlc -nostdlib -I stdlib -I vreplay/src \
    -use-runtime $PWD/runtime/ocamlrun -dllpath $PWD/vreplay/src"
# native executables link the stubs statically: -I vreplay/src doubles as
# -L vreplay/src, resolving the -cclib -lvreplaynat recorded in vreplay.cmxa
OCAMLOPT="runtime/ocamlrun ./ocamlopt -nostdlib -I stdlib -I vreplay/src"

modes="byte"
if [ -f ocamlopt ] && [ -f vreplay/src/vreplay.cmxa ]; then
    modes="byte native"
else
...
cleanup() {
    rm -rf "$TMP"
    rm -f vreplay/tests/cases/*.cmi vreplay/tests/cases/*.cmo \
...
if [ $# -gt 0 ]; then
    cases=""
    for a in "$@"; do cases="$cases vreplay/tests/cases/$a.ml"; done
else
    cases=$(ls vreplay/tests/cases/*.ml)
fi
...
# in CI with no opam switch and no Core installed (see
# vreplay/tests/core_stubs/).
...
stub_units="base__Hashtbl base__Hash_set base__Map base__Set \
    ...
    core__Union_find"
stub_srcs=""
for u in $stub_units; do
    # a unit with an .mli hides its representation, as the library it
    # stands in for does; pass it explicitly or ocamlc ignores it
    if [ -f "vreplay/tests/core_stubs/$u.mli" ]; then
        stub_srcs="$stub_srcs vreplay/tests/core_stubs/$u.mli"
    fi
    stub_srcs="$stub_srcs vreplay/tests/core_stubs/$u.ml"
done
$OCAMLC -a -I vreplay/tests/core_stubs -o "$TMP/stubs.cma" $stub_srcs \
    || { echo "error: cannot build the Base/Core stubs" >&2; exit 2; }
case $modes in *native*)
    $OCAMLOPT -a -I vreplay/tests/core_stubs -o "$TMP/stubs.cmxa" $stub_srcs \
        || { echo "error: cannot build the native Base/Core stubs" >&2
             exit 2; } ;;
esac
...
for mode in $modes; do
  for src in $cases; do
    ...
    if [ "$mode" = native ]; then
        label="$name [native]"
        comp="$OCAMLOPT -visual-replay -I vreplay/tests/core_stubs \
            $TMP/stubs.cmxa"
    else
        label=$name
        comp="$OCAMLC -visual-replay -I vreplay/tests/core_stubs $TMP/stubs.cma"
    fi
    out="$TMP/$name.$mode"
    ...
    if [ $promote -eq 1 ] && [ "$mode" = byte ]; then
        cp "$out.dump" "vreplay/tests/expected/$name.dump"
        echo "PROMOTED $name ($(cat "$out.check"))"
        pass=$((pass + 1)); continue
    fi
    if [ ! -f "vreplay/tests/expected/$name.dump" ]; then
        echo "FAIL $label (no expected dump; run with --promote)"
        failed=$((failed + 1)); continue
    fi
    canon "vreplay/tests/expected/$name.dump" > "$out.expcanon"
...
PY
    ...
    if $OCAMLC -visual-replay -o "$TMP/sock_case.exe" \
        vreplay/tests/cases/queue_basic.ml > /dev/null 2>&1 \
